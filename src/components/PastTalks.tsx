  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-6">
        ...
        <span className="text-base uppercase tracking-[0.2em] text-white-30">
          Earlier Today
        </span>
        <div className="flex-1 h-px bg-white/[0.06]" />
      </div>
      <div className="flex flex-col gap-2">
        {visible.map((item) => (
          <div key={item.id} className="flex items-center gap-4 py-1.5">
            ...
            <span className="text-base text-white/25 font-mono tabular-nums w-16">
              {formatTime(item.start)}
            </span>
            <span className="text-base text-white/25 truncate flex-1">
              {item.title}
            </span>
          </div>
        ))}
