  ...
  return (
    <div className="flex flex-col gap-2 lg:gap-3">
      <div className="flex items-center gap-4 lg:gap-6">
        <div className="flex-1 h-px bg-cyan/[0.10]" />
        <span
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2 sm:gap-4">
        <span className="text-teal/50 text-sm lg:text-lg mt-0.5 shrink-0">✓</span>
        <span className="text-sm lg:text-base text-cyan/40 font-mono tabular-nums w-12 lg:w-16 mt-0.5 shrink-0">
          {formatTime(item.start)}
        </span>
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-2">
            <span className="text-sm lg:text-base text-cyan/40 flex-1">
              {item.title}
            </span>
            {hasAbstract && (
              <span className={`text-cyan/35 text-xs mt-0.5 shrink-0 transition-transform duration-200 ${isExpanded ? "rotate-180" : ""}`}>
                ▼
              </span>
            )}
          </div>
          {isExpanded && (
            <>
              {itemSpeakers.length > 0 && (
                <p className="text-sm text-cyan/45 mt-1 truncate">
                  {itemSpeakers.map((s) => s.name).join(", ")}
                </p>
              )}
              {item.abstract && (
                <p className="text-sm text-cyan/35 leading-relaxed mt-1.5 pr-2">
